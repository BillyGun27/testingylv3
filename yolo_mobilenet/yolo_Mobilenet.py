...
class YOLO(object):
    def __init__(self):
        self.model_path = 'model_data/trained_weights_final_mobilenet.h5' # model path or trained weights path
    ...
    def detect_true_image(self, image):
        start = timer()
        #rects = []
        if self.model_image_size != (None, None):
            assert self.model_image_size[0]%32 == 0, 'Multiples of 32 required'
...
def detect_video(yolo, video_path, output_path=""):
    ...
    while True:
        ...
        image = yolo.detect_true_image(image)
...
if __name__ == '__main__':
    ...
    #r_image = YOLO().detect_true_image(image)
    #r_image.show()

    #detect_video(YOLO(), "library1.mp4" , output_path="")
    detect_video(YOLO(), "car_chase_01.mp4" , output_path="")